...
- IP UNO Q: `192.168.0.105`, user: `arduino`.
- Para autenticación SSH se asume variable de entorno `SSHPASS` ya exportada en la shell local (no en scripts). Comando base: `sshpass -e ssh arduino@192.168.0.105`.
...
Expected: `Initialized empty Git repository in /home/sabas/Documents/electroniccats/i2c_arduinoQ/.git/`

- [ ] **Step 2: Crear .gitignore para excluir notas con timestamps si crecen mucho y backups DTB binarios**

Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/.gitignore` con contenido:
...
Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/remote-ssh.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/remote-ssh.sh
```
...
echo "exit code: $?"
export SSHPASS='arduino1334'  # restaurar
```
...
El script local lanza `remote-ssh.sh` exportando la password como variable de entorno remota (`SSHPASS_REMOTE`), para que el bash remoto pueda usarla con `sudo -S`. Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/diagnose-i2c.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/diagnose-i2c.sh
```
...
Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/notes/soldadura-checklist-<ts>.md` (con timestamp en el nombre) con el siguiente template, e ir tildando físicamente lo que se completa:
...
Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/verify-gt911.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/verify-gt911.sh
```
...
Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/bind-gt911.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/bind-gt911.sh
```

- [ ] **Step 2: Crear script de unbind (cleanup)**

Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/unbind-gt911.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/unbind-gt911.sh
```
...
sshpass -e scp arduino@192.168.0.105:/boot/efi/qrb2210-arduino-imola-gigadisplay.dtb /tmp/uno-q-dtb-${TS}.dtb
...
Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/patch-dtb.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/patch-dtb.sh
```
...
Crear `/home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/install-systemd-bind.sh`:
...
```bash
chmod +x /home/sabas/Documents/electroniccats/i2c_arduinoQ/scripts/install-systemd-bind.sh
```
