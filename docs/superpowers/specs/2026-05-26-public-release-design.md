...
A6. **Relocate maintainer tooling:** `scripts/build-st7701-patched-ko.sh` → `tools/dev/build-panel-module.sh`; remove hardcoded `Arduino1334` / `192.168.0.149` defaults (require env vars or fail clearly).
...
B4. **Verify scrubbing locally** before pushing: `git log -p | grep -E '(arduino1334|Arduino1334|/home/sabas|192\.168\.0\.(105|149|184))'` must return zero hits.
